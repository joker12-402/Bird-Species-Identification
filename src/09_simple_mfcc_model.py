...
class BirdSoundDataset(Dataset):
    def __init__(self, metadata_path, audio_dir, target_size=(128, 128), 
                 is_train=False, augment_prob=0.3):
    ...
    def _compute_feature_stats(self):
        ...
        # 随机采样部分数据计算统计量
        sample_size = min(50, len(self.metadata))
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于MFCC特征的鸟类声纹识别')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/simple_mfcc_model',
                        help='模型输出目录')
    ...
    # 初始化模型
    model = SimpleBirdNet(num_classes=num_classes, in_channels=1).to(device)
