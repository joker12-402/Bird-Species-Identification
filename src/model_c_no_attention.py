...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于MFCC+时域特征+能量特征的鸟类声纹识别 - 无注意力机制版')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_c_no_attention',
                        help='模型输出目录')
    ...
    print(f"Model C (无注意力) 总参数量: {total_params:,}")  # 格式化输出，添加千位分隔符
    ...
    # 开始训练
    print(f"\n开始训练无注意力机制的Model C (MFCC + 时域特征 + 能量特征)...")
    try:
