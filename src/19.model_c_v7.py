...
class EnhancedFeatureAttention(nn.Module):
    ...
    def __init__(self, num_features, reduction_ratio=4):
        ...
        self.attention = nn.Sequential(
            nn.Linear(num_features * 2, num_features // reduction_ratio),
    ...
    def forward(self, x):
        ...
        gmp = self.global_max_pool(x)  # [batch_size, num_features, 1, 1]
        
        # 连接两种池化结果
        gap = gap.view(batch_size, num_features)
        gmp = gmp.view(batch_size, num_features)
        combined = torch.cat([gap, gmp], dim=1)  # [batch_size, num_features * 2]
        
        # 计算注意力权重
        attention_weights = self.attention(combined)  # [batch_size, num_features]
...
# 2. 简化的模型架构
# ------------------------------------------------------------------------------
class SimplifiedBirdNetWithAttention(nn.Module):
    def __init__(self, num_classes, in_channels=3, label_smoothing=0.1):
        super(SimplifiedBirdNetWithAttention, self).__init__()
        self.label_smoothing = label_smoothing
        
        # 特征注意力机制
        self.feature_attention = EnhancedFeatureAttention(in_channels)
...
def extract_mfcc(audio_path, sr=16000, n_mfcc=40, n_fft=2048, hop_length=512):
    ...
    try:
        ...
        return np.zeros((n_mfcc, 100)), np.zeros((n_mfcc, 100))
...
# 4. 数据集类 - 简化版，只使用核心特征
# ------------------------------------------------------------------------------
class SimplifiedBirdSoundDataset(Dataset):
    def __init__(self, metadata_path, audio_dir, target_size=(128, 128), 
                 is_train=False, augment_prob=0.5):  # 增加数据增强概率
    ...
    def _compute_feature_stats(self):
        ...
        for idx in tqdm(sample_indices, desc="计算统计量"):
            mfcc, mfcc_delta, energy = self._extract_features(idx)
            if mfcc is not None:
    ...
    def __getitem__(self, idx):
        ...
        # 堆叠为多通道特征 (只使用MFCC、Delta MFCC和能量特征)
        features = np.stack([
            mfcc, 
            mfcc_delta, 
            energy
    ...
    def _extract_features(self, idx):
        ...
        try:
            # 使用MFCC特征及其一阶差分
            mfcc, mfcc_delta = extract_mfcc(audio_path, n_mfcc=40)
    ...
    def _augment(self, features):
        ...
        if random.random() < 0.7:
            shift = int(augmented.shape[2] * 0.2 * (random.random() - 0.5))
        ...
        if random.random() < 0.5:
            noise = np.random.normal(0, 0.05 * np.std(augmented), augmented.shape)
            augmented += noise
            
        # 频率掩码
        if random.random() < 0.4:
            freq_mask_width = int(augmented.shape[1] * random.uniform(0.05, 0.15))
        ...
        # 时间掩码
        if random.random() < 0.4:
            time_mask_width = int(augmented.shape[2] * random.uniform(0.05, 0.15))
        ...
        if random.random() < 0.3:
            ...
            for i in range(augmented.shape[0]):
                augmented[i] = ndimage.zoom(augmented[i], scale_factor, order=1)
                # 调整回原始尺寸
                if scale_factor > 1:
                    start = (augmented[i].shape[1] - self.target_size[1]) // 2
                    augmented[i] = augmented[i][:, start:start+self.target_size[1]]
                else:
                    padded = np.zeros((augmented[i].shape[0], self.target_size[1]))
                    start = (self.target_size[1] - augmented[i].shape[1]) // 2
                    padded[:, start:start+augmented[i].shape[1]] = augmented[i]
                    augmented[i] = padded
...
def calculate_class_weights(labels):
    ...
    for label, count in label_counts.items():
        # 使用平滑逆频率
        weights[label] = total_samples / (num_classes * (count + 1))
    
    return torch.tensor(weights / weights.sum())

def train_epoch(model, train_loader, criterion, optimizer, device, epoch, use_focal_loss=False):
    """训练单个epoch"""
    model.train()
    running_loss = 0.0
...
def validate(model, val_loader, criterion, device, label_names, use_focal_loss=False):
    """验证模型"""
    model.eval()
    running_loss = 0.0
    all_preds, all_labels = [], []
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于注意力机制的多种特征的鸟类声纹识别 - 改进版')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_c_v7',
                        help='模型输出目录')
    ...
    # 加载数据集
    print("加载数据集...")
    full_dataset = SimplifiedBirdSoundDataset(
        metadata_path=args.metadata,
        audio_dir=args.audio_dir,
        is_train=True,
        augment_prob=0.5  # 增加数据增强概率
    )
    ...
    # 初始化模型 - 注意输入通道数为3（简化后的特征）
    model = SimplifiedBirdNetWithAttention(
        num_classes=num_classes, 
        in_channels=3,  # 简化后的特征
    ...
    # 开始训练
    print(f"\n开始训练改进版Model C (带注意力机制的多特征融合)...")
    try:
        for epoch in range(1, args.epochs + 1):
            # 训练
            train_loss, train_acc, train_f1 = train_epoch(
                model, train_loader, criterion, optimizer, device, epoch, use_focal_loss
            )
            
            # 验证
            val_loss, val_acc, val_f1, _, _, attention_weights = validate(
                model, val_loader, criterion, device, label_names, use_focal_loss
            )
            ...
            if val_f1 > best_val_f1:
                ...
                # 保存注意力权重可视化
                feature_names = ['MFCC', 'MFCC Delta', 'Energy']
    ...
    test_loss, test_acc, test_f1, test_labels, test_preds, attention_weights = validate(
        model, test_loader, criterion, device, label_names, use_focal_loss
    )
    ...
    if attention_weights is not None and len(attention_weights) > 0:
        if len(attention_weights.shape) == 4:  # [batch, channels, 1, 1]
            attention_dict = {
                ...
                'Energy': float(attention_weights[0, 2, 0, 0])
            }
        elif len(attention_weights.shape) == 2:  # [batch, channels]
            attention_dict = {
                'MFCC': float(attention_weights[0, 0]),
                'MFCC Delta': float(attention_weights[0, 1]),
                'Energy': float(attention_weights[0, 2])
            }
        else:
            attention_dict = {
                'MFCC': 1.0,
    ...
    model_info = {
        'model_name': 'SimplifiedBirdNetWithAttention',
        'num_classes': num_classes,
        'input_channels': 3,
        'features': ['MFCC', 'MFCC Delta', 'Energy (Mel-spectrogram)'],
