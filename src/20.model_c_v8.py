...
def validate(model, val_loader, criterion, device, label_names=None):
    ...
    # 计算平均注意力权重 - 修复维度索引问题
    avg_attention = np.mean(all_attention_weights, axis=0)  # 形状: [batch, 3, 1, 1]
...
def main():
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_c_v8_150',
                        help='模型输出目录')
    ...
    print(f"Model C 总参数量: {total_params:,}")  # 格式化输出，添加千位分隔符
