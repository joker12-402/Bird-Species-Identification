...
# ------------------------------------------------------------------------------
# 2. 坐标注意力机制 (Coordinate Attention)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 3. 带坐标注意力和Dropout的ResNet模型
# ------------------------------------------------------------------------------
class ResNetWithAttention(nn.Module):
    def __init__(self, num_classes, model_name='resnet50', pretrained=False, in_channels=2, dropout_rate=0.5):
        ...
        if model_name == 'resnet18' or model_name == 'resnet34':
            ...
            self.ca2 = CoordinateAttention(512)
            self.ca3 = CoordinateAttention(1024)
            self.ca4 = CoordinateAttention(2048)
...
# 4. 特征提取函数 - 修复版
# ------------------------------------------------------------------------------
def butterworth_filter(signal_data, sr, cutoff=7900, order=5):
    """
    巴特沃斯低通滤波器（修复版）
    默认截止频率设为7900Hz，避免等于奈奎斯特频率
    """
    # 校验采样率有效性
    if sr <= 0:
...
def extract_mfcc(audio_path, sr=16000, n_mfcc=13, n_fft=2048, hop_length=512):
    """提取MFCC特征"""
    try:
        ...
        return np.zeros((n_mfcc, 100))  # 返回默认形状的特征

def extract_pncc(audio_path, sr=16000, n_fft=2048, hop_length=512, n_bands=40, n_ceps=13):
    """提取PNCC特征 - 修复版"""
    try:
        y, sr = librosa.load(audio_path, sr=sr)
        ...
        # 应用巴特沃斯滤波 - 使用修复后的函数，默认截止频率设为7900Hz
        y = butterworth_filter(y, sr, cutoff=7900)
        ...
        # DCT变换获取倒谱系数
        pncc = dct(pncc_spec, type=2, norm='ortho', axis=0)[:n_ceps]
        ...
        print(f"提取PNCC特征失败: {audio_path}, 错误: {e}")
        return np.zeros((n_ceps-1, 100))  # 返回默认形状的特征（去除直流分量后）

# ------------------------------------------------------------------------------
# 5. 数据集类 - 优化版
# ------------------------------------------------------------------------------
class BirdSoundDataset(Dataset):
    def __init__(self, metadata_path, audio_dir, target_size=(224, 224), 
                 transform=None, is_train=False, augment_prob=0.5):
        ...
        self.feature_cache = {}
        print("预提取特征...")
        for idx in tqdm(range(len(self.metadata)), desc="预提取特征"):
            self._get_features(idx)  # 预提取并缓存
    ...
    def __getitem__(self, idx):
        ...
        # 将特征分成两个通道
        # 由于25是奇数，我们将其调整为26维（填充一维）
        if features.shape[0] == 25:
            # 在PNCC部分添加一维零填充
            features = np.vstack([features[:13], np.zeros((1, features.shape[1])), features[13:]])
        
        half_dim = features.shape[0] // 2
        ...
        # 合并为两个通道
        features = np.stack([channel1, channel2], axis=0)
    ...
    def _extract_features(self, idx):
        ...
        try:
            ...
            # 确保特征长度一致
            min_len = min(mfcc.shape[1], pncc.shape[1])
            mfcc = mfcc[:, :min_len]
            pncc = pncc[:, :min_len]
            
            # 特征融合
            features = np.concatenate([mfcc, pncc], axis=0)
            return features
        except Exception as e:
            print(f"提取特征失败: {audio_path}, 错误: {e}")
            # 返回随机特征作为后备
            return np.random.randn(25, 100)  # MFCC(13) + PNCC(12) = 25
    ...
    def _augment(self, features):
        ...
        if random.random() < 0.6:
            shift = int(augmented.shape[2] * 0.1 * (random.random() - 0.5))
        ...
        if random.random() < 0.3:
            ...
            end = start + random.randint(augmented.shape[1]//8, augmented.shape[1]//4)
        ...
        # 音量缩放
        if random.random() < 0.3:
            volume_scale = 0.9 + random.random() * 0.2
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于特征融合与注意力机制的鸟类声纹识别')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/mpfc_attention_model_v3',
                        help='模型输出目录')
