...
class EnhancedBirdSoundDataset(Dataset):
    ...
    def _compute_feature_stats(self):
        ...
        for idx in tqdm(sample_indices, desc="计算统计量"):
            mfcc, energy = self._extract_features(idx)
            if mfcc is not None:
    ...
    def __getitem__(self, idx):
        # 提取特征
        mfcc_features, energy_features = self._extract_features(idx)
        ...
        if mfcc_features.shape != energy_features.shape:
            # 如果形状不同，调整能量特征形状以匹配MFCC特征
            energy_features = np.resize(energy_features, mfcc_features.shape)
        
        # 堆叠为双通道特征
        features = np.stack([mfcc_features, energy_features], axis=0).astype(np.float32)
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于MFCC+能量特征的鸟类声纹识别 - Model B')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_b_mfcc_energy_150',
                        help='模型输出目录')
    ...
    print(f"Model B 总参数量: {total_params:,}")  # 格式化输出，添加千位分隔符
    ...
    # 保存模型架构信息
    model_info = {
        'model_name': 'ImprovedBirdNet_ModelB',
        'num_classes': num_classes,
        'input_channels': 2,
        'features': ['MFCC', 'Energy (Mel-spectrogram)'],
