...
class MultiScaleConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        # 移除第四个分支，避免特征图尺寸过小的问题
        self.conv1x1 = nn.Conv2d(3 * branch_channels, out_channels, kernel_size=1)
...
# ------------------------------------------------------------------------------
# 3. 改进的模型架构 - 融合多尺度卷积和双端注意力
# ------------------------------------------------------------------------------
class ImprovedBirdNetWithAttentionV2(nn.Module):
    def __init__(self, num_classes, in_channels=3):
        super(ImprovedBirdNetWithAttentionV2, self).__init__()
        ...
        # 多尺度卷积块
        self.multiscale1 = MultiScaleConvBlock(64, 128)
        self.multiscale2 = MultiScaleConvBlock(128, 256)
        self.multiscale3 = MultiScaleConvBlock(256, 512)
    ...
    def forward(self, x):
        ...
        x = self.multiscale1(x)    # [B, 128, 64, 64]
        x = F.max_pool2d(x, 2)     # [B, 128, 32, 32]
        
        x = self.multiscale2(x)    # [B, 256, 32, 32]
        x = F.max_pool2d(x, 2)     # [B, 256, 16, 16]
        
        x = self.multiscale3(x)    # [B, 512, 16, 16]
        x = F.max_pool2d(x, 2)     # [B, 512, 8, 8]
        
        x = self.dual_attention(x) # 应用双端注意力 [B, 512, 8, 8]
        x = self.global_avg_pool(x) # 全局平均池化 [B, 512, 1, 1]
        x = x.view(x.size(0), -1)  # 展平 [B, 512]
        x = self.classifier(x)     # 分类 [B, num_classes]
        
        return x, None  # 返回None保持与旧代码兼容
...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch):
    ...
    for inputs, labels in pbar:
        ...
        # 前向传播
        optimizer.zero_grad()
        outputs, _ = model(inputs)  # 忽略注意力权重
...
def validate(model, val_loader, criterion, device, label_names=None):
    ...
    return val_loss, val_acc, val_f1, all_labels, all_preds, None
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于多尺度卷积和双端注意力的鸟类声纹识别 - Model C V9')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_c_v9_150',
                        help='模型输出目录')
    ...
    # 初始化模型 - 注意输入通道数为3
    model = ImprovedBirdNetWithAttentionV2(num_classes=num_classes, in_channels=3).to(device)
    ...
    print(f"Model C V9 总参数量: {total_params:,}")
    ...
    # 保存模型信息
    model_info = {
        'model_name': 'ImprovedBirdNetWithAttentionV2',
        'architecture': 'MultiScaleConv + DualAttention',
