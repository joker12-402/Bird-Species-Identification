...
class FeatureAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 全局平均池化
        gap = self.global_avg_pool(x).view(batch_size, num_features)
        
        # 层归一化
        gap = self.layer_norm(gap)
        
        # 计算注意力权重
        attention_weights = self.attention(gap).view(batch_size, num_features, 1, 1)
...
# ------------------------------------------------------------------------------
# 2. 改进的模型架构 - 结合多尺度卷积和注意力机制
# ------------------------------------------------------------------------------
class ImprovedBirdNetWithAttentionV3(nn.Module):
    def __init__(self, num_classes, in_channels=3):
        super(ImprovedBirdNetWithAttentionV3, self).__init__()
...
def validate(model, val_loader, criterion, device, label_names=None):
    ...
    # 根据实际形状调整索引
    if len(avg_attention.shape) == 4:  # [batch, channels, 1, 1]
        print(f"平均注意力权重 - MFCC: {float(avg_attention[0, 0, 0, 0]):.4f}, "
              f"时域: {float(avg_attention[0, 1, 0, 0]):.4f}, "
              f"能量: {float(avg_attention[0, 2, 0, 0]):.4f}")
    elif len(avg_attention.shape) == 3:  # [channels, 1, 1]
        print(f"平均注意力权重 - MFCC: {float(avg_attention[0, 0, 0]):.4f}, "
              f"时域: {float(avg_attention[1, 0, 0]):.4f}, "
              f"能量: {float(avg_attention[2, 0, 0]):.4f}")
    elif len(avg_attention.shape) == 2:  # [batch, channels]
        print(f"平均注意力权重 - MFCC: {float(avg_attention[0, 0]):.4f}, "
              f"时域: {float(avg_attention[0, 1]):.4f}, "
              f"能量: {float(avg_attention[0, 2]):.4f}")
    elif len(avg_attention.shape) == 1:  # [channels]
        print(f"平均注意力权重 - MFCC: {float(avg_attention[0]):.4f}, "
              f"时域: {float(avg_attention[1]):.4f}, "
              f"能量: {float(avg_attention[2]):.4f}")
    else:
        print("无法识别的注意力权重形状")
...
def plot_attention_weights(attention_weights, feature_names, save_path):
    """绘制注意力权重"""
    plt.figure(figsize=(8, 6))
    
    # 根据实际形状调整索引
    if len(attention_weights.shape) == 4:  # [batch, channels, 1, 1]
        weights = [float(attention_weights[0, 0, 0, 0]), 
                   float(attention_weights[0, 1, 0, 0]), 
                   float(attention_weights[0, 2, 0, 0])]
    elif len(attention_weights.shape) == 3:  # [channels, 1, 1]
        weights = [float(attention_weights[0, 0, 0]), 
                   float(attention_weights[1, 0, 0]), 
                   float(attention_weights[2, 0, 0])]
    elif len(attention_weights.shape) == 2:  # [batch, channels]
        weights = [float(attention_weights[0, 0]), 
                   float(attention_weights[0, 1]), 
                   float(attention_weights[0, 2])]
    elif len(attention_weights.shape) == 1:  # [channels]
        weights = [float(attention_weights[0]), 
                   float(attention_weights[1]), 
                   float(attention_weights[2])]
    else:
        print(f"未知的注意力权重形状: {attention_weights.shape}")
        weights = [1.0, 1.0, 1.0]  # 默认值
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于注意力机制的MFCC+时域特征+能量特征的鸟类声纹识别 - 改进版Model C V3')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_c_v10',
                        help='模型输出目录')
    ...
    # 初始化模型 - 注意输入通道数为3
    model = ImprovedBirdNetWithAttentionV3(num_classes=num_classes, in_channels=3).to(device)
    ...
    print(f"Model C V3 总参数量: {total_params:,}")
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            if val_f1 > best_val_f1:
                ...
                # 保存注意力权重可视化
                feature_names = ['MFCC', 'Temporal', 'Energy']
    ...
    if len(attention_weights.shape) == 4:  # [batch, channels, 1, 1]
        attention_dict = {
            ...
            'Temporal': float(attention_weights[0, 1, 0, 0]),
            'Energy': float(attention_weights[0, 2, 0, 0])
        }
    elif len(attention_weights.shape) == 2:  # [batch, channels]
        attention_dict = {
            'MFCC': float(attention_weights[0, 0]),
            'Temporal': float(attention_weights[0, 1]),
    ...
    model_info = {
        'model_name': 'ImprovedBirdNetWithAttentionV3',
