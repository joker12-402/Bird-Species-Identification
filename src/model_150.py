...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于MFCC特征的鸟类声纹识别 - 基线模型')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/baseline_mfcc_150',
                        help='模型输出目录')  # 修改输出目录
    parser.add_argument('--epochs', type=int, default=150, help='训练轮数')  # 从50改为150
    parser.add_argument('--batch_size', type=int, default=32, help='批次大小')
    parser.add_argument('--lr', type=float, default=0.001, help='初始学习率')
    parser.add_argument('--patience', type=int, default=15, help='早停耐心值')  # 从10改为15
    ...
    # 初始化模型
    model = ImprovedBirdNet(num_classes=num_classes, in_channels=1).to(device)
    ...
    # 保存模型架构信息
    model_info = {
        'model_name': 'ImprovedBirdNet_Baseline',
