...
#     print(f"平均注意力权重 - MFCC: {float(avg_attention[0, 0, 0]):.4f}, 时域: {float(avg_attention[0, 1, 0]):.4f}, 能量: {float(avg_attention[0, 2, 0]):.4f}")
...
#     plt.bar(feature_names, [float(attention_weights[0, 0, 0]), float(attention_weights[0, 1, 0]), float(attention_weights[0, 2, 0])])
#     plt.title('特征注意力权重')
#     plt.ylabel('权重')
...
# # ------------------------------------------------------------------------------
# def main():
#     parser = argparse.ArgumentParser(description='基于注意力机制的MFCC+时域特征+能量特征的鸟类声纹识别 - 改进版Model C')
...
#                         help='标签映射JSON路径')
#     parser.add_argument('--output_dir', type=str, default='D:/paper/results/improved_model_c',
#                         help='模型输出目录')
...
#     # 初始化模型 - 注意输入通道数为3
#     model = ImprovedBirdNetWithAttention(num_classes=num_classes, in_channels=3).to(device)
...
#     # 开始训练
#     print(f"\n开始训练改进版Model C (带注意力机制的MFCC + 时域特征 + 能量特征)...")
#     try:
...
#     # 保存模型架构信息
#     model_info = {
#         'model_name': 'ImprovedBirdNetWithAttention',
#         'num_classes': num_classes,
#         'input_channels': 3,
#         'features': ['MFCC', 'Temporal (RMSE)', 'Energy (Mel-spectrogram)'],
#         'attention_weights': {
#             'MFCC': float(attention_weights[0, 0, 0]),
#             'Temporal': float(attention_weights[0, 1, 0]),
#             'Energy': float(attention_weights[0, 2, 0])
#         },
...
class FeatureAttention(nn.Module):
    """特征注意力机制，用于加权不同特征通道的重要性"""
    def __init__(self, num_features, reduction_ratio=1):
        super(FeatureAttention, self).__init__()
        self.num_features = num_features
        self.reduction_ratio = reduction_ratio
        ...
        # 注意力机制：确保输出维度与输入特征通道数一致
        self.attention = nn.Sequential(
            nn.Flatten(),  # 显式展平操作，避免维度模糊
            nn.Linear(num_features, num_features // reduction_ratio),
            nn.ReLU(inplace=True),
            nn.Linear(num_features // reduction_ratio, num_features),  # 输出维度=输入通道数
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # 全局平均池化
        gap = self.global_avg_pool(x)  # 形状: [batch_size, num_features, 1, 1]
        
        # 计算注意力权重
        attention_weights = self.attention(gap)  # 形状: [batch_size, num_features]
        attention_weights = attention_weights.view(batch_size, num_features, 1, 1)  # 调整为4D张量
...
def validate(model, val_loader, criterion, device, label_names=None):
    ...
    # 计算平均注意力权重 - 修复维度索引问题
    avg_attention = np.mean(all_attention_weights, axis=0)  # 形状: [3, 1, 1]
...
def plot_attention_weights(attention_weights, feature_names, save_path):
    """绘制注意力权重"""
    plt.figure(figsize=(8, 6))
    plt.bar(feature_names, [float(attention_weights[0, 0, 0]), 
...
def main():
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/improved_model_c_doubao',
                        help='模型输出目录')
    parser.add_argument('--epochs', type=int, default=50, help='训练轮数')
    parser.add_argument('--batch_size', type=int, default=32, help='批次大小')
    parser.add_argument('--lr', type=float, default=0.001, help='初始学习率')
    parser.add_argument('--patience', type=int, default=10, help='早停耐心值')
    parser.add_argument('--reduction_ratio', type=int, default=1,  # 默认值改为1，确保能整除3
                        help='注意力机制缩减比（必须能整除3）')
    ...
    _, dummy_weights = model(dummy_input)
    print(f"注意力权重形状验证: {dummy_weights.shape} (预期: [1, 3, 1, 1])")
    ...
    model_info = {
        ...
        'attention_weights': {
            'MFCC': float(attention_weights[0, 0, 0]),
            'Temporal': float(attention_weights[1, 0, 0]),
            'Energy': float(attention_weights[2, 0, 0])
        },
        'class_names': label_names,
        'test_accuracy': test_acc,
        'test_f1': test_f1,
