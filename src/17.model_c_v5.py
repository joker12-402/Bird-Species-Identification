...
def validate(model, val_loader, criterion, device, label_names=None):
    ...
    if len(avg_attention.shape) == 4:  # [batch, channels, 1, 1]
        print(f"平均注意力权重 - MFCC: {float(avg_attention[0, 0, 0, 0]):.4f}, 时域: {float(avg_attention[0, 1, 0, 0]):.4f}, 能量: {float(avg_attention[0, 2, 0, 0]):.4f}")
    elif len(avg_attention.shape) == 2:  # [batch, channels]
        print(f"平均注意力权重 - MFCC: {float(avg_attention[0, 0]):.4f}, 时域: {float(avg_attention[0, 1]):.4f}, 能量: {float(avg_attention[0, 2]):.4f}")
    else:
        print(f"未知的注意力权重形状: {avg_attention.shape}")
...
def main():
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_c_v5',
                        help='模型输出目录')
    ...
    # 使用ReduceLROnPlateau学习率调度，基于验证集F1分数调整
    ...
    model_info = {
        'model_name': 'ImprovedBirdNetWithAttention_Enhanced',
