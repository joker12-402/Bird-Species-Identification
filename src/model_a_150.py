...
class EnhancedBirdSoundDataset(Dataset):
    ...
    def _compute_feature_stats(self):
        ...
        for idx in tqdm(sample_indices, desc="计算统计量"):
            mfcc, temporal = self._extract_features(idx)
            if mfcc is not None:
    ...
    def __getitem__(self, idx):
        # 提取特征
        mfcc_features, temporal_features = self._extract_features(idx)
        ...
        if mfcc_features.shape != temporal_features.shape:
            # 如果形状不同，调整时域特征形状以匹配MFCC特征
            temporal_features = np.resize(temporal_features, mfcc_features.shape)
        
        # 堆叠为双通道特征
        features = np.stack([mfcc_features, temporal_features], axis=0).astype(np.float32)
    ...
    def _extract_features(self, idx):
        ...
        try:
            ...
            return mfcc, temporal
        except Exception as e:
            print(f"提取特征失败: {audio_path}, 错误: {e}")
            # 返回随机特征作为后备
            return np.random.randn(40, 100), np.random.randn(100)
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='基于MFCC+时域特征的鸟类声纹识别 - Model A')
    ...
    parser.add_argument('--label_mapping', type=str, default='D:/paper/data/features/label_mapping.json',
                        help='标签映射JSON路径')
    parser.add_argument('--output_dir', type=str, default='D:/paper/results/model_a_mfcc_temporal_150_2',
                        help='模型输出目录')
    ...
    print(f"Model A 总参数量: {total_params:,}")  # 格式化输出，添加千位分隔符
    ...
    # 保存模型架构信息
    model_info = {
        'model_name': 'ImprovedBirdNet_ModelA',
        'num_classes': num_classes,
        'input_channels': 2,
        'features': ['MFCC', 'Temporal (RMSE)'],
